//! Детерминированный компилятор контекста (Rust-порт TS-модуля).
//!
...
/// оценку по частям, сохраняя упаковку O(n) вместо O(n²).
...
/// Разобранная сущность: один JSON-парсинг на сущность вместо
...
/// Релевантность по заранее подготовленным термам запроса:
...
pub fn compile_context(entities: &[ContextEntity], query: &ContextQuery) -> ContextPack {
    ...
    // Термы запроса токенизируются один раз на всю компиляцию.
    ...
    //
    // Оценка инкрементальная и целочисленная: части заголовка и
...
/// Процессный кеш последнего скомпилированного контекста.
...
mod tests {
    ...
    fn compile_context_over_ten_thousand_entities_is_fast() {
        ...
        // константные накладные. Реальные p95 проверяются release-тестами.
        assert!(
